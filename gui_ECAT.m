function varargout = gui_ECAT
% SCHIRANG_GUI is the function that creates (or bring to focus) ECAT GUI.
% Then, main_ECAT is always called to start each task. It is the
% "main" program.
...
if ~isempty(figPtr) % Figure exists so brings it to the focus
    ...
    handles.(b_stopsignal.tag) = uicontrol(handles.uipanel_Task,...
        ...
        'Callback',@main_ECAT,...
        'Tooltip','Aron 2006 JournalOfNeuroscience - Cortical and Subcortical Contributions to Stop Signal Response Inhibition: Role of the Subthalamic Nucleus.pdf');
    ...
    handles.(b_check_img.tag) = uicontrol(handles.uipanel_Task,...
        'Style','pushbutton',...
        'Units', 'Normalized',...
        'Position',[b_check_img.x b_check_img.y b_check_img.w b_check_img.h],...
        'String','Check images',...
        'BackgroundColor',buttonBGcolor,...
        'Tag',b_check_img.tag,...
    ...
    r_Pre.x   = p_tk.xpos(p_tk.count);
    r_Pre.y   = buttun_y_check;
    r_Pre.w   = p_tk.xwidth(p_tk.count)/2;
    r_Pre.h   = buttun_h_check;
    r_Pre.tag = 'radiobutton_Pre';
    handles.(r_Pre.tag) = uicontrol(handles.uipanel_Task,...
        'Style','radio',...
        'Units', 'Normalized',...
    ...
    r_Post.x   = p_tk.xpos(p_tk.count) + p_tk.xwidth(p_tk.count)/2;
    r_Post.y   = buttun_y_check;
    r_Post.w   = p_tk.xwidth(p_tk.count)/2;
    r_Post.h   = buttun_h_check;
    r_Post.tag = 'radiobutton_Post';
    handles.(r_Post.tag) = uicontrol(handles.uipanel_Task,...
        'Style','radio',...
        'Units', 'Normalized',...
